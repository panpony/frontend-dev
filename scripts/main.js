...
_paceDone(){
  ...
  _textAnimation(el, inview){//第一引数が交差中のDOM(entry.target) 第二引数がtrue,false
  if (inview) {
      const ta = new TweenTextAnimation(el); //trueの場合はTextAnimationにentry.targetが渡る
      ta.animate();
    }
  }
  ...
  _scrollInit(){
    this.observers = new ScrollObserver('.nav-trigger', this._navAnimation.bind(this), {once: false});
      //bindしないとnew ScrollObserverのthisがheaderにいくことになる。
